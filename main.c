...
static gboolean
h_has_to_download_xml (const gchar *path)
{
  ...
  guint64 t_file = g_file_info_get_attribute_uint64 (info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
  guint64 t_now  = g_get_real_time () / (1000 * 1000);
  g_object_unref (info);

  if ((t_now - t_file) > 60 * 60 * 3)
    return TRUE;
  
  return FALSE; //Fresh XML!
}
...
h_init_version_info_list (int *preferred)
{
  ...
  gchar *preferred_chksum = NULL;

  *preferred = 0; //default - latest

  gchar *pref = g_build_path ("\\", path, "pref.txt", NULL);
  if (g_file_test (pref, G_FILE_TEST_EXISTS))
    g_file_get_contents (pref, &preferred_chksum, NULL, NULL);
  g_free (pref);

  if (h_has_to_download_xml (versions_xml))
    {
  ...
  if (g_markup_parse_context_parse(context, buf, len, NULL))
  {
    ...
    *ver = (version_info) {
    ...
    //get preferred
    size_t i = 0;
    for (GList *elem = lst;elem != NULL; elem = elem->next, i++)
    {
      version_info *ver = (version_info *) elem->data;
      if (g_strcmp0(ver->chksum, preferred_chksum) == 0)
      {
        *preferred = i;
        break;
      }
...
h_get_info_label()
{
  ...
  const gchar *text =
    ...
    "코딩/번역 감수 : <a href=\"https://github.com/khronii\">[출처_필요]</a>\n"
...
static GtkWidget*
h_get_version_combo_box(int preferred)
{
  GtkWidget *cbox;
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(cbox), preferred);
                         ...
                         gpointer   user_data)
{
  ...
  gchar *pref = g_build_path ("\\", localappdata, "HoloCureKR", "pref.txt", NULL);
  version_info *selected_real = (version_info *) g_list_nth_data (ver_lst, sel);
  g_file_set_contents (pref, selected_real->chksum, strlen (selected_real->chksum), NULL);
  g_free (pref);
  ...
  gchar *game_exe  = g_build_path ("\\", localappdata, "HoloCureKR", ctx->info->chksum, "HoloCure.exe", NULL);
  gboolean is_exe = g_file_test (game_exe, G_FILE_TEST_EXISTS);
  g_free (game_exe);
  if (is_exe)
  {
  ...
  if (no_game)
    {
      ...
      gchar *path_unzip = g_build_path ("\\", cdir, "unzip.exe", NULL);
      gchar *unzip_arg[] = {path_unzip, "-a", zip_path, "-d", game_path, NULL};
  ...
  if (pid != 0)
    {
      ...
      gchar *zip_name = g_strconcat(ctx->info->chksum,".zip",NULL);
      gchar *zip_path = g_build_path ("\\", localappdata, "HoloCureKR", zip_name, NULL);
      GFile *zip_file = g_file_new_for_path ("zip_path");
      
      g_file_delete (zip_file, NULL, NULL);

      g_object_unref (zip_file);
      g_free (zip_name);
      g_free (zip_path);
    }
          ...
          gpointer        user_data)
{
  ...
  GtkWidget *cb = h_get_version_combo_box(preferred);
